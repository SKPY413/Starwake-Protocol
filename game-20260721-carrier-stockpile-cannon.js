...
(() => {
    ...
    const UPGRADE_DATA = Object.freeze({
        ...
        cannonDamage:   { label: "CANNON POWER", icon: "▰", system: "red", tier: "core", category: "offense", accent: "#ffd27a", description: "Unlocks and strengthens a slow, clean heavy cannon fired alongside the primary weapon.", baseCost: 150, growth: 1.58 },
        ...
        cannonPenetration: { label: "CANNON PENETRATION", icon: "⇥", system: "red", tier: "core", category: "offense", accent: "#fff0bc", description: "Heavy shells pass through one additional enemy per level.", baseCost: 225, growth: 1.72, maxLevel: 6 },
    ...
    function applyUpgrade(type) {
        const upgradeActions = {
            ...
            cannonDamage: () => {
                player.cannonDamage = player.cannonDamage > 0 ? player.cannonDamage + 9 : 34;
            ...
            cannonPenetration: () => {
                player.cannonPenetration = Math.min(6, player.cannonPenetration + 1);
                ui.weapon.textContent = `Cannon penetration ${player.cannonPenetration}`;
            },
            speed: () => {
                player.speed += 0.28;
    ...
    function getUpgradeStatText(type, next = false) {
        ...
        const statMap = {
            ...
            cannonDamage: () => `${next ? "Next" : "Current"}: ${player.cannonDamage > 0 ? player.cannonDamage + (next ? 9 : 0) : (next ? 34 : "offline")} damage`,
            cannonRate: () => `${next ? "Next" : "Current"}: ${Math.max(500, player.cannonFireRate - (next ? 110 : 0))} ms delay`,
            cannonPenetration: () => `${next ? "Next" : "Current"}: ${player.cannonPenetration + (next ? 1 : 0)} extra target${player.cannonPenetration + (next ? 1 : 0) === 1 ? "" : "s"}`,
    ...
    function createCannonShell(angle) {
        bullets.push({
            ...
            r: 7,
            dx: Math.cos(angle) * 10.5,
            dy: Math.sin(angle) * 10.5,
            damage: player.cannonDamage,
            explosive: false,
            penetrationRemaining: player.cannonPenetration,
            hitEnemyIds: new Set(),
        });
    }
    ...
    function chooseEnemyType() {
        ...
        if (state.wave >= unlock(11) && roll < miniBossChance) return "miniTank";
    ...
    function updateCarrierSystems(enemy, now) {
        ...
        enemy.nextMissileManufactureAt = enemy.nextMissileManufactureAt || now;
        ...
        for (let bulletIndex = bullets.length - 1; bulletIndex >= 0; bulletIndex--) {
            ...
            for (let enemyIndex = enemies.length - 1; enemyIndex >= 0; enemyIndex--) {
                ...
                if (bullet.kind === "cannon") {
                    ...
                    if ((bullet.penetrationRemaining || 0) > 0) {
                        bullet.penetrationRemaining--;
                        bullet.x += bullet.dx * 0.8;
                        bullet.y += bullet.dy * 0.8;
                    } else {
                        bullet.dead = true;
                    }
                } else {
    ...
    const SAVE_PLAYER_FIELDS = Object.freeze([
        ...
        "cannonDamage","cannonFireRate","cannonPenetration","lastCannonShotAt",
