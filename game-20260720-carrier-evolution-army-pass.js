...
(() => {
    ...
    function getCarrierDoctrine() {
        // Carriers deploy recognizable assault waves. Count grows by 3% per
        // wave after their normal unlock while cadence tightens by 1% per wave.
        // Hard caps preserve late-game performance and prevent exponential floods.
        const scalingWaves = Math.max(0, state.wave - 14);
        const volley = Math.min(56, Math.max(24, Math.round(24 * Math.pow(1.03, scalingWaves))));
        const volleyCooldown = Math.max(1800, 2750 * Math.pow(0.99, scalingWaves));
        return {
            initial: volley,
            volley,
            activeCap: Math.min(PERFORMANCE_LIMITS.maxCarrierMissiles, Math.max(48, volley * 2)),
            volleyCooldown,
            globalCooldown: 650,
            chargeTime: 650,
            cannonShots: state.difficulty === "impossible" ? 3 : state.difficulty === "hard" ? 2 : 1,
            cannonSpread: state.difficulty === "impossible" ? 0.10 : state.difficulty === "hard" ? 0.08 : 0,
        };
    }
