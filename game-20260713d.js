...
(() => {
    ...
    function restoreWaveResearchUI() {
        ...
        document.querySelectorAll("[data-wave-bonus]").forEach(button => {
            const selected = waveResearch.claimed && button.dataset.waveBonus === waveResearch.selectedId;
            button.disabled = waveResearch.claimed;
    ...
    const END_WAVE_BONUSES = Object.freeze([
        { id: "researchCache", label: "RESEARCH CACHE", description: "Recover additional upgrade points immediately.", apply: () => { state.upgradePoints += 100 + state.wave * 18; } },
        { id: "fieldRepair", label: "FIELD REPAIR", description: "Restore 35% hull and fully recharge nanobot shields.", apply: () => { player.health = Math.min(player.maxHealth, player.health + player.maxHealth * 0.35); player.shield = player.maxShield; } },
        { id: "combatCalibration", label: "COMBAT CALIBRATION", description: "Permanent minor calibration: +3 damage and +0.12 movement speed.", apply: () => { player.damage += 3; player.speed += 0.12; } },
        { id: "autonomousTune", label: "AUTONOMOUS TUNING", description: "Permanent autonomous calibration: +8 missile and aura damage.", apply: () => { player.missileDamage += 8; player.auraDamage += 8; } },
    ]);
    ...
    function prepareEndWaveResearch() {
        ...
        const offset = state.wave % END_WAVE_BONUSES.length;
        const choices = [0, 1, 2].map(index => END_WAVE_BONUSES[(offset + index) % END_WAVE_BONUSES.length]);
        panel.innerHTML = choices.map(bonus => `<button type="button" class="wave-research-choice" data-wave-bonus="${bonus.id}"><strong>${bonus.label}</strong><span>${bonus.description}</span></button>`).join("");
        panel.closest(".wave-research-panel")?.classList.remove("claimed");
    }

    function claimEndWaveResearch(id) {
        if (waveResearch.claimed) return;
        const bonus = END_WAVE_BONUSES.find(item => item.id === id);
        if (!bonus) return;
        captureReconstructionSnapshot(`Research: ${bonus.label}`);
        bonus.apply();
        waveResearch.claimed = true;
        waveResearch.selectedId = id;
        document.querySelectorAll("[data-wave-bonus]").forEach(button => {
            button.disabled = true;
            button.classList.toggle("selected", button.dataset.waveBonus === id);
        });
